<template>
  <div class="donation__record__container">
    <div>
      搜索框
    </div>
    <div class="record__main">
      <div>
        <vs-table striped style="width: 1100px" v-model="record">
          <template #thead>
            <vs-tr>
              <vs-th style="width: 80px"> 捐款ID </vs-th>
              <vs-th> 捐款人地址 </vs-th>
              <vs-th> 捐款金额 </vs-th>
              <vs-th> 交易方式 </vs-th>
              <vs-th> 捐款来源 </vs-th>
              <vs-th> 交易状态 </vs-th>
              <vs-th style="width: 200px"> 交易时间 </vs-th>
            </vs-tr>
          </template>
          <template #tbody>
            <vs-tr v-for="(item, index) in getPage(tableData,queryParams.pageNum,queryParams.pageSize)" :key="index" :data="item">
              <vs-td>
                {{ item.donationId }}
              </vs-td>
              <vs-td>
          ...
          <template #footer>
            <div id="trace" ref="trace" style="width: 400px;height: 200px">
...
import {selectUserDonationRaiseFundRecord} from "@/api/charity/record.js";
...
const {tableData,queryParams,option} = toRefs(data)

watch(record,(val,old) => {
  console.log(val)
  var traceEcharts = echarts.init(trace.value);
  traceEcharts.setOption(option.value)
})

onMounted(() => {
  selectUserDonationRaiseFundRecord().then(res => {
